...
def generar_respuesta_premium(mensaje: str, historial: list, estado: dict) -> str:
    try:
        ...
        if not estado.get("nombre"):
            prompt_base += f"\nPrimer contacto: ¡Hola! Bienvenido a Volkswagen Eurocity Culiacan. Soy Alex, tu asistente de ventas. ¿Cuál es tu nombre y buscas un auto nuevo o usado?"
...
async def webhook(mensaje: Mensaje):
    try:
        ...
        # Reset state if this is a new conversation (e.g., "hola" as first message)
        if texto == "hola" and not estado.get("nombre"):
        ...
        ignored_inputs = ["hola", "me interesa un auto", "no se", "no sé", "okey", "hey", "si", "sí", "si esta bien", "claro", "ok"]
        if "nombre" not in estado and texto not in ignored_inputs:
            actualizar_estado(cliente_id, {"nombre": texto.title()})
        elif "nombre" in estado and "tipo_auto" not in estado and texto in ["nuevo", "usado"]:
            actualizar_estado(cliente_id, {"tipo_auto": texto})
