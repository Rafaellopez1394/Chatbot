...
try:
    ...
    # Prueba de escritura inicial
    test_collection = db["test"]
    result = test_collection.insert_one({"test": "prueba_conexion", "fecha": datetime.now()})
    logger.info(f"Prueba de escritura exitosa en colección 'test': ID={result.inserted_id}")
except ServerSelectionTimeoutError as e:
    logger.error(f"Fallo en la conexión a MongoDB: {str(e)}")
    raise
except WriteError as e:
    logger.error(f"Error de escritura en MongoDB durante inicialización: {str(e)}")
    raise
except Exception as e:
    logger.error(f"Error inesperado al conectar o escribir en MongoDB: {str(e)}")
    raise
...
def inicializar_asesores():
    try:
        ...
        if not existing or not all("telefono" in doc and "activo" in doc for doc in existing):
            ...
            result = asesores_col.insert_many(asesores_data)
            logger.info(f"Asesores inicializados correctamente: {asesores_data}, IDs={result.inserted_ids}")
        else:
            logger.info("La colección de asesores ya está correctamente poblada")
    except WriteError as e:
        logger.error(f"Error de escritura al inicializar asesores: {str(e)}")
    except Exception as e:
        logger.error(f"Error inesperado al inicializar asesores: {str(e)}")
...
def actualizar_estado(cliente_id: str, nuevo_estado: dict):
    try:
        result = estado_conversacion.update_one(
            {"_id": cliente_id}, {"$set": nuevo_estado}, upsert=True
        )
        logger.debug(f"Estado actualizado para {cliente_id}: {nuevo_estado}, Resultado: matched={result.matched_count}, modified={result.modified_count}, upserted={result.upserted_id}")
    except WriteError as e:
        logger.error(f"Error de escritura al actualizar estado para {cliente_id}: {str(e)}")
    except Exception as e:
        logger.error(f"Error inesperado al actualizar estado para {cliente_id}: {str(e)}")
...
# Parsear entrada para extraer nombre, tipo de auto y tipo de vehículo
def parsear_entrada(texto: str) -> tuple:
    ...
    for palabra in palabras:
        if palabra not in ignored_inputs and not any(p in palabra for p in ["nuevo", "usado", "suv", "sedán", "sedan", "compacto"]):
    ...
    if any(p in texto for p in ["suv", "todoterreno"]):
        tipo_vehiculo = "suv"
    elif any(p in texto for p in ["sedán", "sedan"]):
        tipo_vehiculo = "sedán"
    elif "compacto" in texto:
...
async def webhook(mensaje: Mensaje):
    try:
        ...
        if "nombre" in estado and "tipo_auto" in estado and "tipo_vehiculo" not in estado and tipo_vehiculo:
            ...
            logger.debug(f"Tipo de vehículo establecido para {cliente_id}: {tipo_vehiculo}")
        elif "nombre" in estado and "tipo_auto" in estado and "tipo_vehiculo" in estado and "modelo" not in estado:
            for model in ['jetta', 'tiguan', 'virtus', 'taos', 'teramont', 't-cross', 'polo']:
                if model in texto:
                    ...
                    break
        elif all(k in estado for k in ["nombre", "tipo_auto", "tipo_vehiculo", "modelo"]) and texto in ["sí", "si"]:
